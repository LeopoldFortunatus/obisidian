...
```
$ make build
tsh scp -r target/vmcontroller  aleksander.rykalin@cloud-runner-3.el.wb.ru:/tmp/target
cloud-runner$ scp -r /tmp/target/vmcontroller c1-az1.az1.n-cl-3348-spicedb-test-permissions-checks-7.mr.cloud.devel:
cloud-runner$ ssh c1-az1.az1.n-cl-3348-spicedb-test-permissions-checks-7.mr.cloud.devel
...
#### update vmctl config to run only on one host
#### test with vmctl
...
ID:
```
in token exhange: SHA256:wTM0uVFytsV0ywyKahQUpHeOKfKGIKOKclW5MywWydk
in bash:
arykalin@arykalin-Dell-G15-5511:~/go/src/gitlab-private.wildberries.ru/cloud/local-configs/jwt$ ssh-keygen -i -m PKCS8 -f jwtRS256.key.pub > public.openssh
arykalin@arykalin-Dell-G15-5511:~/go/src/gitlab-private.wildberries.ru/cloud/local-configs/jwt$ ssh-keygen -lf public.openssh
4096 SHA256:wTM0uVFytsV0ywyKahQUpHeOKfKGIKOKclW5MywWydk no comment (RSA)
